...
var _ = Describe("MetricsRepository.GetWorkspaceResourceUsage", func() {
	...
	It("returns ErrWorkspaceNotFound when the workspace does not exist", func() {
		// Usage is resolved by pod label, so without an existence check this would be
		// indistinguishable from a paused workspace and wrongly report WORKSPACE_NOT_RUNNING.
		client := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(testWorkspaceCR()).
